...
productRouter.get('/products', async (reg: Request, res : Response) => {
    try{
...
productRouter.post("/product/", async (req: Request , res : Response ) =>{
    try {
        ...
        if(!name || !price || !quantity || !image) { 
            return res.status(StatusCodes.BAD_REQUEST).json({error : `Please provide all the required paramaters `})
        }
